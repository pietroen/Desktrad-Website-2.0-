...
if (lightbox) {
    ...
    const lightboxLanguages = lightbox.querySelector('#lightbox-languages');
    const lightboxScope = lightbox.querySelector('#lightbox-scope');
    const projectScopes = {
        documentacao: 'Análise do material, padronização terminológica e revisão de manuais, especificações ou documentos regulatórios para uso no mercado de destino.',
        interpretacao: 'Preparação de vocabulário e contexto para mediação clara em reuniões, conferências, treinamentos e decisões profissionais em tempo real.',
        legendagem: 'Tradução do roteiro, adaptação para leitura, sincronização com a imagem e conferência final para acessibilidade e compreensão.',
        narracao: 'Adaptação do roteiro, orientação de ritmo e gravação de voz para entregar um conteúdo natural, claro e alinhado ao público.'
    };
    const translatedScopes = {
        en: {
            documentacao: 'Material analysis, terminology standardization and review of manuals, specifications or regulatory documents for the target market.',
            interpretacao: 'Vocabulary and context preparation for clear mediation in meetings, conferences, training sessions and professional decisions in real time.',
            legendagem: 'Script translation, reading adaptation, image synchronization and final review for accessibility and understanding.',
            narracao: 'Script adaptation, pace direction and voice recording to deliver natural, clear content aligned with its audience.'
        },
        es: {
            documentacao: 'Análisis del material, normalización terminológica y revisión de manuales, especificaciones o documentos regulatorios para el mercado de destino.',
            interpretacao: 'Preparación de vocabulario y contexto para una mediación clara en reuniones, conferencias, capacitaciones y decisiones profesionales en tiempo real.',
            legendagem: 'Traducción del guion, adaptación para lectura, sincronización con la imagen y revisión final para accesibilidad y comprensión.',
            narracao: 'Adaptación del guion, orientación del ritmo y grabación de voz para entregar un contenido natural, claro y alineado con el público.'
        },
        zh: {
            documentacao: '分析材料、统一术语并审校手册、规格或法规文件，使其适用于目标市场。',
            interpretacao: '准备术语和背景，为会议、研讨会、培训及专业决策提供清晰的实时语言协调。',
            legendagem: '翻译脚本、调整阅读节奏、同步画面，并进行最终审校以确保无障碍理解。',
            narracao: '调整脚本、指导节奏并录制配音，交付自然、清晰且符合受众的内容。'
        }
    };
    const projectTranslations = {
        en: {
            'Documentação Técnica': ['Technical Documentation', 'Industrial manuals prepared for operation, training and maintenance.'],
            'Interpretação Simultânea': ['Simultaneous Interpreting', 'Language mediation for conferences and professional communication in real time.'],
            'Legendagem Corporativa': ['Corporate Subtitling', 'Audiovisual content adapted for training, communication and accessibility.'],
            'Narração Profissional': ['Professional Voice-over', 'Voice and script aligned to present a message clearly in another language.'],
            'Tradução Juramentada': ['Sworn Translation', 'Official documents handled faithfully and according to the purpose of the process.'],
            'Interpretação Consecutiva': ['Consecutive Interpreting', 'Executive meeting with terminology preparation for decisions without noise.'],
            'Conteúdo Farmacêutico': ['Pharmaceutical Content', 'Scientific and regulatory materials with terminological consistency.'],
            'Vídeo de Produto': ['Product Video', 'Technical subtitling synchronized to demonstrate features and benefits.'],
            'Treinamento Multilíngue': ['Multilingual Training', 'Voice-over and script adaptation for training teams in different markets.']
        },
        es: {
            'Documentação Técnica': ['Documentación Técnica', 'Manuales industriales preparados para operación, capacitación y mantenimiento.'],
            'Interpretação Simultânea': ['Interpretación Simultánea', 'Mediación lingüística para conferencias y comunicación profesional en tiempo real.'],
            'Legendagem Corporativa': ['Subtitulación Corporativa', 'Contenido audiovisual adaptado para capacitación, comunicación y accesibilidad.'],
            'Narração Profissional': ['Locución Profesional', 'Voz y guion alineados para presentar un mensaje con claridad en otro idioma.'],
            'Tradução Juramentada': ['Traducción Jurada', 'Documentos oficiales tratados con fidelidad y atención al objetivo del proceso.'],
            'Interpretação Consecutiva': ['Interpretación Consecutiva', 'Reunión ejecutiva con preparación terminológica para decisiones sin ruido.'],
            'Conteúdo Farmacêutico': ['Contenido Farmacéutico', 'Materiales científicos y regulatorios con consistencia terminológica.'],
            'Vídeo de Produto': ['Vídeo de Producto', 'Subtitulación técnica sincronizada para demostrar funciones y beneficios.'],
            'Treinamento Multilíngue': ['Capacitación Multilingüe', 'Locución y adaptación de guion para capacitar equipos en distintos mercados.']
        },
        zh: {
            'Documentação Técnica': ['技术文档', '为操作、培训和维护准备的工业手册。'],
            'Interpretação Simultânea': ['同声传译', '为会议和专业沟通提供实时语言协调。'],
            'Legendagem Corporativa': ['企业字幕', '为培训、沟通和无障碍体验改编的视听内容。'],
            'Narração Profissional': ['专业配音', '配音与脚本协调，让信息以另一种语言清晰呈现。'],
            'Tradução Juramentada': ['认证翻译', '根据流程目的准确处理官方文件。'],
            'Interpretação Consecutiva': ['交替传译', '为执行会议准备术语，帮助决策清晰进行。'],
            'Conteúdo Farmacêutico': ['医药内容', '保持术语一致性的科学与法规材料。'],
            'Vídeo de Produto': ['产品视频', '同步技术字幕，清晰展示功能和优势。'],
            'Treinamento Multilíngue': ['多语言培训', '为不同市场的团队培训进行配音和脚本适配。']
        }
    };
    ...
    document.querySelectorAll('.portfolio-link').forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            const language = localStorage.getItem('desktrad-lang') || 'pt';
            const originalTitle = item.dataset.title || 'Projeto do portfólio';
            const translatedProject = projectTranslations[language]?.[originalTitle];
            const displayTitle = translatedProject ? translatedProject[0] : originalTitle;
            lightboxImg.src = imgSrc;
            lightboxImg.alt = displayTitle;
            const caption = lightbox.querySelector('.lightbox-caption');
            const description = lightbox.querySelector('.lightbox-description');
            if (caption) caption.textContent = `${displayTitle} · ${item.dataset.languages || ''}`;
            if (description) description.textContent = translatedProject ? translatedProject[1] : item.dataset.description || '';
            if (lightboxCategory) lightboxCategory.textContent = translatedCategories[language]?.[item.dataset.category] || categoryLabels[item.dataset.category] || 'PROJETO';
            if (lightboxLanguages) lightboxLanguages.textContent = item.dataset.languages || 'Idiomas sob consulta';
            if (lightboxScope) lightboxScope.textContent = translatedScopes[language]?.[item.dataset.category] || projectScopes[item.dataset.category] || 'Escopo definido a partir do material, dos idiomas e do objetivo de comunicação.';
...
// Service details use lightweight dialogs so every card has a useful action.
...
serviceButtons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        if (modal) { modal.style.display = 'flex'; modal.setAttribute('aria-hidden', 'false'); return; }
        const card = button.closest('.service-card');
        if (!card) return;
        const generatedModal = document.createElement('div');
        generatedModal.className = 'modal';
        generatedModal.style.display = 'flex';
        generatedModal.innerHTML = `<div class="modal-content"><button class="modal-close" type="button" aria-label="Fechar">&times;</button><h2>${card.querySelector('h3').textContent}</h2><p>${card.querySelector('p').textContent}</p></div>`;
        document.body.appendChild(generatedModal);
        generatedModal.querySelector('.modal-close').addEventListener('click', () => generatedModal.remove());
        generatedModal.addEventListener('click', event => { if (event.target === generatedModal) generatedModal.remove(); });
    });
});
